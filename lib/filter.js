...
function mermaid (type, value, _format, _meta) {
  ...
  const fullCmd = `${cmd}  ${options.confFileOpts} ${options.puppeteerOpts} -w ${options['viewport-width'] || process.env.MERMAID_FILTER_WIDTH || 800} -s ${options.scale} -f -i "${tmpfileObj.name}" -t ${options.theme} -b ${options.background} -o "${savePath}"`  exec(fullCmd)
  ...
  if (!options.caption) {
    return pandoc.Para(
      [pandoc.Image(
        [id, imageClasses, attrs[2]], // <-- Passes original attributes (like width) to Pandoc
        [],
        [newPath, fig]
      )]
    );
  ...
  return {
    t: 'Figure',
    c: [
      [id, imageClasses, attrs[2]], // <-- Passes attributes to the Figure environment
      [
        null, 
      ...
      [
        pandoc.Plain([
          pandoc.Image(
            ['', [], attrs[2]], // <-- CRITICAL: Passes attributes to \includegraphics
