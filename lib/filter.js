...
function mermaid (type, value, _format, _meta) {
  ...
  const fullCmd = `${cmd}  ${options.confFileOpts} ${options.puppeteerOpts} -w ${options['viewport-width'] || process.env.MERMAID_FILTER_WIDTH || 800} -s ${options.scale} -f -i "${tmpfileObj.name}" -t ${options.theme} -b ${options.background} -o "${savePath}"`;
  exec(fullCmd);
